...
if independent != 'Enter independent variable name':
    ...
    print(
        ...
        f'Probability that output will take value of 1 when x = {x_input} is {logit_prob(log_reg, x_input)[0]}.')  # Alternatively, "LogisticRegression().predict_proba" method can be used
    ...
    data_source = 'Enter data source'  # Full path to a CSV file
    data = pd.read_csv(data_source)
    data = pd.get_dummies(data, drop_first=True)
    print(data.columns)
    # print(data.sample(5))
    X_train, X_test, y_train, y_test = train_test_split(data.drop('Enter column name of y', axis=1), data['Enter column name of y'])
    log_reg = LogisticRegression()
    log_reg.fit(X_train, y_train)
    # Predicting the output with certain input assumptions (e.g., x1=number, x2='string',...)
    predicted_y = log_reg.predict(np.array([['Enter x parameters inside the list, separated by commas']]))[0]
